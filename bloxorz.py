...
def next_position(pos, action, reverse_direction=False):
    ...
    # assume x1, x2 ,y1, y2 are ordered, as x1 <x2, y1 < y2
    ...
    x1, y1 = b1
    x2, y2 = b2
    if x1 == x2 and y1 == y2:  # block is up
        block_status = 'up'
    if x1 == x2 and y1 != y2:  # block is vertical
        block_status = 'vert'
    if x1 != x2 and y1 == y2:  # block is horiz
        block_status = 'horiz'
    if action == 'U':
        if block_status == 'up':
            y1 = y1 - 2
            y2 = y2 - 1
        if block_status == 'vert':
            y1 = y1 - 1
            y2 = y2 - 2
        if block_status == 'horiz':
            y1 = y1 - 1
            y2 = y2 - 1
    if action == 'D':
        if block_status == 'up':
